...
def draw_person_pose(orig_img, poses):
    ...
    for pose in poses.round().astype('i'):
        ...
        #print("8x:",x8,"8y",y8)
        #print("9x:",x9,"9y",y9)
        #print("10x:",x10,"10y",y10)
        #print("11x:",x11,"11y",y11)
        #print("12x:",x12,"12y",y12)
        #print("13x:",x13,"13y",y13)
        #print("14x:",x14,"14y",y14)
        #print("15x:",x15,"15y",y15)
        #print("16x:",x16,"16y",y16)
        #print("17x:",x17,"17y",y17)

        a=(x4+x7)/2 #手中点x
        b=(y4+y7)/2 #手中点y

        c=math.sqrt(pow((x4-x3),2)+pow((y4-y3),2)) #右の実際の長さ
        d=math.sqrt(pow((x7-x6),2)+pow((y7-y6),2)) #左の実際の長さ
        e=math.sqrt(pow((a-x3),2)+pow((b-y3),2)) #右の近似の長さ
        f=math.sqrt(pow((a-x6),2)+pow((b-y6),2)) #左の近似の長さ

        if x4<x3 and x6<x7:
            print("ありえないよ！") 
        elif abs(x4-x3)<10 and abs(x7-x6)<10:
            print("良くないよ！")
        elif abs(e-c)<11  and abs(f-d)<11:
            print("手拍子かも！")
...
if __name__ == '__main__':
    ...
    for photo in range(10):
        ...
        # read image
        img = cv2.imread('clap1-photo/clap1_{0:03d}.png'.format(photo))
        ...
        cv2.imwrite('clap1-bone/clap1_bone_{0:03d}.png'.format(photo), img)
