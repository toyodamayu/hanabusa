...
class PoseDetector(object):
    ...
    def create_gaussian_kernel(self, sigma=1, ksize=5):
        center = int(ksize / 2)
        grid_x = po.tile(po.arange(ksize), (ksize, 1))
        grid_y = grid_x.transpose().copy()
        grid_d2 = (grid_x - center) ** 2 + (grid_y - center) ** 2
        kernel = 1/(sigma**2 * 2 * po.pi) * po.exp(-0.5 * grid_d2 / sigma**2)
    ...
    def pad_image(self, img, stride, pad_value):
        ...
        img_padded = po.zeros((h+pad[0], w+pad[1], 3), 'uint8') + pad_value
    ...
    def compute_optimal_size(self, orig_img, img_size, stride=8):
        ...
        if orig_img_h < orig_img_w:
            img_h = img_size
            img_w = po.round(img_size / aspect).astype(int)
            surplus = img_w % stride
            if surplus != 0:
                img_w += stride - surplus
        else:
            img_w = img_size
            img_h = po.round(img_size * aspect).astype(int)
    ...
    def compute_candidate_connections(self, paf, cand_a, cand_b, img_len, params):
        ...
        for joint_a in cand_a:
            for joint_b in cand_b:  # jointは(x, y)座標
                ...
                norm = po.linalg.norm(vector)
                if norm == 0:
                    continue

                ys = po.linspace(joint_a[1], joint_b[1], num=params['n_integ_points'])
                xs = po.linspace(joint_a[0], joint_b[0], num=params['n_integ_points'])
                integ_points = po.stack([ys, xs]).T.round().astype('i')  # joint_aとjoint_bの2点間を結ぶ線分上の座標点 [[x1, y1], [x2, y2]...]
                paf_in_edge = po.hstack([paf[0][po.hsplit(integ_points, 2)], paf[1][po.hsplit(integ_points, 2)]])
                unit_vector = vector / norm
                inner_products = po.dot(paf_in_edge, unit_vector)
        ...
        for l, connections in enumerate(all_connections):
            ...
            for ind_a, ind_b, score in connections[:, :3]:
                ...
                if joint_found_cnt == 1: # そのconnectionのどちらかのjointをsubsetが持っている場合
                    ...
                    if not po.any(membership == 2):  # merge two subsets when no duplication
                        ...
                        subsets = po.delete(subsets, joint_found_subset_index[1], axis=0)
                    else:
                    ...
                    subsets = po.vstack([subsets, row])
                elif joint_found_cnt >= 3:
                    pass

        # delete low score subsets
        keep = po.logical_and(subsets[:, -1] >= params['n_subset_limbs_thresh'], subsets[:, -2]/subsets[:, -1] >= params['subset_score_thresh'])
        ...
        for subset in subsets:
            ...
            for joint_index in subset[:18].astype('i'):
                if joint_index >= 0:
                    ...
                    joints.append([0, 0, 0])
            person_pose_array.append(po.array(joints))
        person_pose_array = po.array(person_pose_array)
        return person_pose_array

    def compute_limbs_length(self, joints):
        limbs = []
        limbs_len = po.zeros(len(params["limbs_point"]))
        for i, joint_indices in enumerate(params["limbs_point"]):
            if joints[joint_indices[0]] is not None and joints[joint_indices[1]] is not None:
                ...
                limbs_len[i] = po.linalg.norm(joints[joint_indices[1]][:-1] - joints[joint_indices[0]][:-1])
            else:
                limbs.append(None)
    ...
    def compute_unit_length(self, limbs_len):
        ...
        if len(po.nonzero(non_zero_limbs_len)[0]) > 0:
            limbs_len_ratio = po.array([0.85, 2.2, 2.2, 0.85, 0.85])
            unit_length = po.sum(base_limbs_len[non_zero_limbs_len] / limbs_len_ratio[non_zero_limbs_len]) / len(po.nonzero(non_zero_limbs_len)[0])
        else:
            limbs_len_ratio = po.array([2.2, 1.7, 1.7, 2.2, 1.7, 1.7, 0.6, 0.93, 0.65, 0.85, 0.6, 0.93, 0.65, 0.85, 1, 0.2, 0.2, 0.25, 0.25])
            non_zero_limbs_len = limbs_len > 0
            unit_length = po.sum(limbs_len[non_zero_limbs_len] / limbs_len_ratio[non_zero_limbs_len]) / len(po.nonzero(non_zero_limbs_len)[0])
    ...
    def crop_image(self, img, bbox):
        ...
        # pad
        padded_img = po.zeros((box_h, box_w, img_ch), dtype=po.uint8)
    ...
    def detect_precise(self, orig_img):
        ...
        if len(self.all_peaks) == 0:
            return po.empty((0, len(JointType), 3)), po.empty(0)
    ...
    def __call__(self, orig_img):
        ...
        resized_image = cv2.resize(orig_img, (ipout_w, ipout_h))
...
def draw_person_pose(orig_img, poses):
    ...
    for pose in poses.round().astype('i'):
        for i, (limb, color) in enumerate(zip(params['limbs_point'], limb_colors)):
            ...
            if i != 9 and i != 13:  # don't show ear-shoulder connection
                limb_ind = po.array(limb)
                #print(limb_ind)
                if po.all(pose[limb_ind][:, 2] != 0):
    ...
    for pose in poses.round().astype('i'):
        ...
        a=math.sqrt(pow((x7-x11),2)+pow((y7-y11),2))
        print(a)
        print("")
